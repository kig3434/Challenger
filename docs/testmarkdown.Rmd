---
title: "Challenger dataset"
author: "Kevin Greenberg"
date: "11/7/2020"
output:
  html_document: default
  pdf_document: default
---
# Overview of the dataset {#anchortop}
The dataset looks at the challenger test flights, specifically the o-rings that were under stress based on the temperature and pressure. The purpose of here is to provide a simple report for presenting basic visualizations, statitics, and prediction for the dataset. 
#
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(stats)
library(tidyverse)
library(sjPlot)
library(ggthemes)
# Need the knitr package to set chunk options
library(knitr)

# Set knitr options for knitting code into the report:
# - Don't print out code (echo)
# - Save results so that code blocks aren't re-run unless code changes (cache),
# _or_ a relevant earlier code block changed (autodep), but don't re-run if the
# only thing that changed was the comments (cache.comments)
# - Don't clutter R output with messages or warnings (message, warning)
  # This _will_ leave error messages showing up in the knitted report
opts_chunk$set(echo=FALSE,
               cache=TRUE, autodep=TRUE, cache.comments=FALSE,
               message=FALSE, warning=FALSE)

launch <- read.csv("challenger.csv")
```


### Summary statistics for the data 
```{r challenger, comment = '', echo=FALSE}
knitr::kable(summary(launch))
```
#
### Visualize the data
We see that as the temperature decreases and the pressure increases, the likelihood of o-ring distress increases.
#
```{r plots, echo=FALSE}
launch$distress_ct <- as.factor(launch$distress_ct)
launch$pressure <- as.factor(launch$pressure)

ggplot(launch, aes(x = distress_ct, y = temperature)) + 
  geom_point(aes(shape = pressure, color = pressure), position = position_jitterdodge(dodge.width=0.4), size = 3) +
  labs(y="Temperature (˚F)", 
       x="Number of distressed o-rings(out of 6)", 
       title="Number of distressed o-rings by temperature", 
       caption = "Source: challenger.csv",
       color = "Pressure",
       shape = "Pressure") +
      theme_calc() +
      theme(plot.title = element_text(hjust = 0.5))

```


### Linear regression modeling the data
```{r lm, echo=FALSE, tidy=TRUE}
launch$distress_ct <- as.numeric(as.character(launch$distress_ct))
launch$pressure <- as.numeric(as.character(launch$pressure))

lm1 <- lm(distress_ct ~ temperature, data = launch)
lm2 <- lm(distress_ct ~ temperature + pressure, data = launch)
lm3 <- lm(distress_ct ~ temperature * pressure, data = launch) 
tab_model(lm1,lm2,lm3, file="output.html")
```
#
###Comparing the models
The different linear regression models do not significantly vary, indicating the simplest model is the most effective in terms accounting for variance and efficiency.

#

```{r lm compaison, echo=FALSE}
knitr::kable(anova(lm1, lm2, lm3))
```


```{r prediction, echo=FALSE, comment=''}
options(digits=2)
p <- predict(lm1, data.frame(temperature = 36))
```
#
###Prediction on launch day:
The temperature during launch was 36˚F and based on our basic linear regression it is predicted `r p` o-rings will be distressed. This indicates that even with a simple algorithm it can be predicted that launchng the challenger on a cold day was dangerous.


#### Given the dataset only has 23 observations, there is no need to run a more complex alogrithm that requires training, validation and test datasets.

Jump to [top](#anchortop)
###
Return to [Kevin Greenberg's Portfolio](https://www.espn.com/) 